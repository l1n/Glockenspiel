...
function usage() {
    console.log('Usage: node rm.js file [webroot]');
    console.log('Examp: node rm.js file.txt /some/path');
}
...
function remove(name, path, fileTree, connection, callback) {
    ...
    if (body.total_count !== 0) {
        ...
        connection.log.debug('removeFile '+name+' '+body.entries[0].id);
        var curl = 'curl -X DELETE https://upload.box.com/api/2.0/files/'+body.entries[0].id+' -H "Authorization: Bearer '+connection.access_token+'" -F file=@';
        ...
        connection.getFolderInfo(0, function (body) {
            ...
            setTimeout(function() {
                ...
                fs.writeFile('gsms.json', JSON.stringify(gsms), function(err) {
                    remove(options.file, options.webroot, gsms.fileTree, connection,
                            function (err) {
